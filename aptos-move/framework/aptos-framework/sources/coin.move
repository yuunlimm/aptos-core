...
module aptos_framework::coin {
    ...
    const PAIRED_FUNGIBLE_ASSET_METADATA_SEED: vector<u8> = b"paired_fungible_asset_metadata_seed";
        ...
        if (!smart_table::contains(&map.coin_to_fungible_asset_map, type)) {
            let metadata_object_cref =
                if (type_info::type_name<CoinType>() == string::utf8(b"0x1::aptos_coin::AptosCoin")) {
                    ...
                    let default_paired_metadata_address = object::create_object_address(&coin_address<CoinType>(), PAIRED_FUNGIBLE_ASSET_METADATA_SEED);
                    if (!object::object_exists<Metadata>(default_paired_metadata_address)) {
                        object::create_named_object(&create_signer::create_signer(coin_address<CoinType>()), PAIRED_FUNGIBLE_ASSET_METADATA_SEED)
                    } else {
    ...
    fun fungible_asset_to_coin<CoinType>(fungible_asset: FungibleAsset): Coin<CoinType> acquires CoinInfo, PairedCoinType {
        ...
        assert!(
            object::object_exists<PairedCoinType>(metadata),
            error::not_found(ENO_PAIRED_COIN)
        );
        let coin_type_info = borrow_global<PairedCoinType>(metadata).type;
