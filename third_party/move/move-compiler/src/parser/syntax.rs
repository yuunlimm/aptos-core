...
fn parse_access_specifier_list(
    context: &mut Context,
    ctor: &impl Fn(NameAccessChain, Option<Vec<Type>>, AddressSpecifier) -> AccessSpecifier_,
) -> Result<Vec<AccessSpecifier>, Box<Diagnostic>> {
    let mut chain = vec![];
    loop {
        chain.push(parse_access_specifier(context, ctor)?);
        if context.tokens.peek() == Tok::Comma {
            context.tokens.advance()?;
...
fn parse_access_specifier(
    ...
    Ok(sp(loc, (*ctor)(name_chain, type_args, address)))
}
