...
fn access_specifier_list_as_acquires(
    ...
    for specifier in access_specifiers.unwrap_or_default() {
        if let AccessSpecifier_::Acquires(chain, type_args, address) = specifier.value {
            if type_args.is_some() || !matches!(address.value, AddressSpecifier_::Empty) {
                ...
                if ok {
                    if let Some(maccess) = name_access_chain(context, Access::Type, chain) {
...
fn access_specifier(context: &mut Context, specifier: P::AccessSpecifier) -> E::AccessSpecifier {
    let (kind, chain, type_args, address) = match specifier.value {
        AccessSpecifier_::Acquires(chain, type_args, address) => {
            (file_format::AccessKind::Acquires, chain, type_args, address)
        },
        AccessSpecifier_::Reads(chain, type_args, address) => {
            (file_format::AccessKind::Reads, chain, type_args, address)
        },
        AccessSpecifier_::Writes(chain, type_args, address) => {
            (file_format::AccessKind::Writes, chain, type_args, address)
        },
    };
